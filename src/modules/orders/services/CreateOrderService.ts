...
export default class CreateOrderService{
    public async execute(data: IOrderDto): Promise<Order>{
        ...
        if(produtos === undefined || produtos === null || produtos.length <= 0)
            throw new AppError("Informe um produto");
                
        if(data.clienteId <= 0) throw new AppError("Informe um cliente para o Pedido.");
        ...
        for(let i=0;i<p.length;i++){
            if(p[i].quantidade <= 0) throw new AppError("informe a quantidade para os produtos.");
